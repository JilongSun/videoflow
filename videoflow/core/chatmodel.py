        ...
        messages = [
            {
                ...
                "content": [
                    ...
                    {
                        "text": f"""你是一位视频编辑工作流的视频分析师，由于视频编辑大模型只能处理5秒的视频，因此，你需要说出视频中{object}出现的开始时间和结束时间.
                     由于你只能看到一些帧数的图片，因此，当看到一些非整数时间时，可以扩大时间，要求结构化输出，结构如下:
                     由列表套列表的形式[[开始时间, 结束时间], [开始时间, 结束时间], ...]，最后一段可以小于5秒，比如一个13秒的视频，{object}在第1.2秒出现直到结束，可以返回：
